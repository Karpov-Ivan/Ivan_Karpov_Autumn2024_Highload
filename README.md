...
![Highload Ivan Karpov SCHEME](https://github.com/Karpov-Ivan/Ivan_Karpov_Autumn2024_Highload/blob/main/images/SCHEME_1.png)
...
*Graceful Degradation* - в случае отказа части системы, сервисы могут продолжать работать с ограниченным функционалом, обеспечивая минимальную работоспособность (например, при сбое ElasticSearch, поиск осуществляется через другие базы данных).
...
*Circuit Breaker* - реализация паттерна Circuit Breaker с использованием Nginx или других инструментов позволяет предотвратить перегрузку сервера, который не может обработать запросы. В случае слишком большого числа запросов с одного или нескольких IP-адресов, система будет замедлять обработку или игнорировать запросы.

### Масштабируемость
*Автоматическое масштабирование* - системы мониторинга и оркестрации, такие как Kubernetes, позволяют автоматически увеличивать или уменьшать количество реплик компонентов в зависимости от нагрузки.
...
*Outbox паттерн* - предотвращает потерю сообщений и обеспечивает их надежную доставку.

*Kafka и Saga* - использование этих технологий для обеспечения согласованности данных и координации транзакций между микросервисами в распределенной среде.
