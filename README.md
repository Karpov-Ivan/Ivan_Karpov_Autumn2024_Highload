...
Profile, Address, Company, Product, Category, File, Comment, Shopping_Cart_Item, Order_Info, Moderation | PostgreSQL | Реляционные таблицы с отношениями и строгой структурой данных. PostgreSQL предоставляет мощные возможности для работы с транзакциями и обеспечит целостность данных (например, связи между профилями, товарами, заказами и прочим).
Product_Stats, Product_Rating, Comment_Stats | MongoDB   | MongoDB - NoSQL база данных, которая подходит для хранения динамических, быстро изменяющихся данных, таких как статистика товаров, рейтинги и комментарии. MongoDB предоставляет flexible schema, что позволяет легко добавлять новые поля в документы, а также хорошую масштабируемость и производительность.
