...
![Highload Ivan Karpov DB](https://github.com/Karpov-Ivan/Ivan_Karpov_Autumn2024_Highload/blob/main/images/DB_WB_5.png)
...
Profile, Address, Company, Product, Category, Comment, Order_Info, Moderation, Country, City, Street, House, Apartment | PostgreSQL | Реляционные таблицы с отношениями и строгой структурой данных. PostgreSQL предоставляет мощные возможности для работы с транзакциями и обеспечит целостность данных (например, связи между профилями, товарами, заказами и прочим).
File, Shopping_Cart_Item | MongoDB | MongoDB идеально подходит для хранения неструктурированных данных. Она обеспечивает гибкость в организации данных, позволяет эффективно работать с большими объемами информации и масштабироваться при необходимости. 
...
----------    | -------------------- | ------------------------------------------------ | ---------------------------------------------------------------
PostgreSQL    |	Product              |	По product_id                                    |	Мастер-репликация: чтение из реплик, запись в основной инстанс.
PostgreSQL    | Product_Stats        |	По product_id	                                   | Мастер-репликация: чтение из реплик, запись в основной инстанс.
PostgreSQL    | Product_Rating       |	По product_id	                                   |	Мастер-репликация: чтение из реплик, запись в основной инстанс.
PostgreSQL    | Comment              |	По product_id	                                   |	Мастер-репликация: чтение из реплик, запись в основной инстанс.
PostgreSQL    | Order_Info           |	По profile_id	                                   |	Мастер-репликация: чтение из реплик, запись в основной инстанс.
...
Данные:
* Пиковое значение по RPS для всего трафика: 54 890 RPS
...
```
54,890RPS / 77,427RPS = 0.71 серверов
```
...
* Redis - используется для хранения сессий (in-memory). 6 серверов (1 master + 2 replicas), с учетом 55.2 млн пользователей.
* PostgreSQL - pеляционные таблицы с отношениями. Кластер из 40 серверов: 4 master + 16 slave + реплики, с учетом высокой доступности и транзакционной нагрузки.
* ClickHouse - для аналитики и быстрого чтения больших данных. Подойдет кластер из 12 серверов (16 CPU, 64 GB RAM).
...
* MongoDB - для MongoDB потребуется кластер из 6 серверов (2 master + 4 реплики).
* AWS S3 - 500 серверов с дисками объемом 10 ТБ.

База данных   |	Серверы |	CPU (на сервер) |	RAM (на сервер) |	ROM (на сервер)
------------- | ------- | --------------- | --------------- | ---------------
Redis         |	6       |	2               |	32 GB           | 20 GB
PostgreSQL    |	40      |	24              |	512 GB	         | 5 TB
ClickHouse    |	12      |	16              |	64 GB           |	10-20 TB
Elasticsearch |	10      |	16              |	64 GB           |	2 TB
Kafka         |	12      |	16              |	64 GB           |	2 TB
MongoDB       |	6       | 24              |	64 GB           |	5 TB
AWS (S3)      | 500     | 64              | 512 GB          | 100 TB
