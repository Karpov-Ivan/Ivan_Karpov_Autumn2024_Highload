...
* Открытие карточки товара: 10 товаров * (6.3 кБ + 7 * 86 кБ) = 10 товаров * 2461.2 кБ = 24.612 МБ.
...
4 * (6.3 (кБ) + 7 * 86 (кБ)) + 3 * (6.3 (кБ) + 7 * 86 (кБ)) + 2 * (0.06 (кБ) + 3 * 86 (кБ)) + 20 * (6.3 кБ + 7 * 86 кБ) + 4 * (6.3 кБ + 7 * 86 кБ) + 2 * (6.3 кБ + 7 * 86 кБ) + 10 * (6.3 кБ + 7 * 86 кБ) = 93378,02 кБ = 93.378 МБ
...
* 10 товаров/день, 24.612 МБ (как рассчитано ранее). Итого: 24.612 МБ

9. Трафик на одного пользователя в день:
...
Открытие карточки товара      | 24.2 млн * 24612 кБ = 595.61 ТБ       | 6.89            | 11.4374          | 13.78                             |
Итого                         | 1 661 930 ГБ ≈ 2257.54 ТБ	            | 26.1144         | 43.3449          | 52.2308                           |
...
----------| --------------------------| ---------------------------------------------------------------------------
Россия    |	Москва, Санкт-Петербург	  | 93.05% пользователей из России, оптимизация для западной части страны
Россия    |	Новосибирск	              | Покрытие для пользователей Центральной России, Урала, Сибири
Россия    |	Владивосток или Хабаровск	| Оптимизация доступа для пользователей восточных регионов и Дальнего Востока
Беларусь  |	Минск	                    | Поддержка пользователей Беларуси (2.06%)
Казахстан |	Алматы, Нур-Султан	       | Покрытие для пользователей Казахстана (1.74%)
Армения   |	Ереван	                   | Поддержка пользователей Армении
...
Тип запроса                  |	RPS (Запад России) |	RPS (Центр России) |	RPS (Восток России) |	RPS (Беларусь)	| RPS (Казахстан) |	RPS (Армения)
-----------------------------| -------------------| -------------------| --------------------| ---------------| ----------------| -------------
...
* Пользователь из России отправляет запрос на wildberries.ru.
* DNS-сервер будет направлять пользователей на ближайший ДЦ с учетом их географического расположения. Для пользователей в центральной России и Сибири трафик будет перенаправляться в Новосибирск, а пользователи из западной части России будут обслуживаться в Москве или Санкт-Петербурге. Восточные пользователи будут направлены в Владивосток или Хабаровск.
* Если российские датацентры перегружены, можно перенаправить запросы на ближайший доступный ДЦ, например, в Беларусь или Казахстан.
...
* Пользователь отправляет запрос на cdn.wildberries.ru.
* Протокол Anycast направляет запрос в ближайший CDN-сервер (например, в Москве, если пользователь из России).
* Если датацентр перегружен или недоступен, Anycast перенаправляет запрос к следующему ближайшему датацентру, например, в Беларусь.
